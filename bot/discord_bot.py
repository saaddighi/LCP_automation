...
async def cohort_start(interaction: discord.Interaction):
    ...
    for row in data[1:]:
        if len(row) > 15 and row[15] == user_discord_id:
            user_cohort = row[3] if len(row) > 3 else None
            user_status = row[4] if len(row) > 4 else None
            break
